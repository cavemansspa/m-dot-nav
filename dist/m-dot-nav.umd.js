(function (factory) {
  ...
  var parse$1 = function(url) {
  	...
  	return {
  		...
  		params: queryIndex < 0
  			? {}
  			: parse$2(url.slice(queryIndex + 1, queryEnd)),
  	}
  };
  ...
  const Layout = () => {
      ...
      return {
          view: ({attrs, children}) => {
              ...
              return [
                  mithril('header', {key: 1}),
                  mithril(SectionMain, {key: theKey}, children)
              ]
          },
      ...
      function SectionMain({attrs: initialAttrs}) {
          ...
          return addLifecycles('section-main', {
              view: ({attrs, children}) => mithril(selector, {'data-value-key': attrs.key}, children),
          })
      }
  };
  ...
  function slideUpIn(transitionState) {
      ...
      let slideUpInClass1 = bss({
          transform: "translateY(100%)"
      });
      let slideUpInClass2 = bss({
  ...
  mithril.nav(document.body, '/', {
      "/": {
          view: ({attrs}) => {
              console.log('/::view()', attrs);
              return [
                  mithril('div', {style: 'height:100%; background-color: aliceblue;'}, [
                      'hello world via m.nav.init(): ',
                      mithril('div', mithril(mithril.route.Link, {href: "/foo?bar=1"}, '/foo')),
                      mithril('div', mithril(mithril.route.Link, {href: "/bar"}, '/bar')),
                      mithril('div', mithril(mithril.route.Link, {href: "/list"}, '/list')),
                      mithril('div', mithril(mithril.route.Link, {href: "/redirect"}, '/redirect'))
                  ])
              ]
          }
      },
      "/foo": {
          view: ({attrs}) => {
              ...
              return [
                  mithril('div', {style: 'height:100%; background-color: antiquewhite;'}, '/foo')
              ]
          }
      },
      "/bar": {
          view: () => {
              return [
                  mithril('div.ui.button', {
                      onclick: (e) => {
                          e.redraw = false;
                          mithril.nav.setRoute('/baz', null, {state: {moo: 'cow'}}, (ts) => {
                              console.log(ts);
                              slideUpIn(ts);
                          });
                      }
                  }, 'Slide Up In'),
                  mithril('div', {style: 'height:100%; background-color: aqua;'}, '/bar')
              ]
          }
      },
      "/redirect": {
          ...
          view: () => {
              return [
                  mithril('div.ui.button', {
                      onclick: (e) => {
                          e.redraw = false;
                          mithril.nav.setRoute('/bar', null, {state: {meow: 'cat'}}, (ts) => {
                              console.log(ts);
                              slideDownOut(ts);
                          });
                      }
                  }, 'Slide Down Out'),
                  mithril('div', {style: 'height:100%; background-color: aquamarine;'}, '/baz')
              ]
          }
      },
      "/list": (function () {
              ...
              view: () => {
                  return [
                      mithril(
                          ...
                          {
                              oncreate: ({dom}) => {
                                  mithril.nav.addEventListener(
                                      "onbeforeroutechange",
                                      saveScrollPosition
                                  );
                              },
                              onremove: () => {
                                  mithril.nav.removeEventListener(
                                      "onbeforeroutechange",
                                      saveScrollPosition
                                  );
                              }
